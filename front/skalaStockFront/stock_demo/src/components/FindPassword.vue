...
const findpw = async () => {
    ...
    const url = `/api/player/${playerId.value}/pw`;
    const requestBody = {
    ...
    if (response.data) {
      alert(`비밀번호 : ${response.data.playerPw}`);
      router.push('/'); 
    } 
  } catch (error) {
    console.error('Signup error:', error);
    errorMessage.value = '아이디나 확인 문자를 다시 확인해주세요.';
  } finally {
...
<template>
  <div class="stock-container">
    <div class="stock-card">
      ...
      <form @submit.prevent="findpw" class="login-form">
